...
from transformers.models.clip.modeling_clip import CLIPPreTrainedModel, CLIPTextModel, CLIPVisionModel
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text, image_process='flat'):
        if image_process == 'flat':
            bsz, n_frames, c, h, w = image.shape
            image = image.view(bsz*n_frames, c, h, w)
            image_embed = self.encode_image(image)
            # image_embed = image_embed.view(bsz, n_frames, image_embed.shape[-1])
