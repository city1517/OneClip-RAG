...
GT_FILE=PATH-TO-videomme/anns.csv
ANNO_PATH=annos/Video-MME/clip_full.json
...
if [ "$EVAL_ONLY" = False ]; then
    for IDX in $(seq 1 $CHUNKS); do
        ...
        CUDA_VISIBLE_DEVICES=$CHUNK_GPUS_STR python3 OneClip-RAG/eval/model_videomme.py \
            --model-path $CKPT \
            --video_dir $VIDEO_DIR \
            --gt_file $GT_FILE \
            --output_dir ./work_dirs/eval_videomme/$SAVE_DIR \
            --output_name pred \
            --num-chunks $CHUNKS \
            --chunk-idx $(($IDX - 1)) \
            --topk $TOPK \
